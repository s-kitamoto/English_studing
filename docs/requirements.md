...
**セッション内の配分**: 復習が滞留していても新規に着手できるよう、1セッションの **25% を新規に確保**する（復習が少ない日はその枠も復習で埋める）。確保した新規はセッション末尾に固めず、復習カードの間に均等に挟む。
...
| Phase | 内容 | 前提確認 |
|---|---|---|
| **1 (MVP)** | シート初期化（Items / Reviews / Settings）/ GAS WebApp（ホーム・登録[手入力]・一覧・未整備キュー・復習[フラッシュカード]・設定）/ SM-2 / 発音（`speechSynthesis`） | ✅ **実装済み** |
